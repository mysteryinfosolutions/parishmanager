    ...
    <body class="fp-page" style="background-color: #a2a2a245">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            ...
                            <div id="edit" style="display: none">
                                <form id="wizard_with_validation" action="database_config" method="POST">
                                    <fieldset>
                                        ...
                                        <div class="form-group form-float">
                                            <div class="form-line">
                                                <input type="text" class="form-control" id="password" name="password" value="<?php echo isset($db_config) ? $db_config['password'] : null; ?>">
                                                <label class="form-label">Password *</label>
                                            </div>
                                        </div>
                                        <div class="form-group form-float">
                                            <div class="form-line">
                                                <input type="text" class="form-control" id="db_name" name="databasename" value="<?php echo isset($db_config) ? $db_config['database'] : null; ?>" required>
                    ...
                    if (isset($database)) {
                        if (empty($database['status'])) {
                            ?>
                            <div class="align-center">
                                <button class="btn btn-warning" onclick="createDbConfirm()">CREATE DATABASE</button>
                            </div>
                            <?php
                        } else {
                            ?>
                            <div class="align-center">
                                <button class="btn btn-danger" onclick="dropDbConfirm()">DROP DATABASE</button>
                            </div>
                            <?php
                        }
                    }
        ...
        <script>
                            ...
                            async function checkConnection() {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: base_url + "/ParishManagerfinal/checkMysqlConnectionAjax",
                            ...
                            function createDbConfirm() {
                                var dbname = document.getElementById('db_name').value;
                                swal({
                                    title: "Create Database !",
                                    text: "Are you sure you want to create database <strong>" + dbname + "</strong> ?",
                                    type: "warning",
                                    html: true,
                                    showCancelButton: true,
                                    closeOnConfirm: false,
                            ...
                            async function createDB() {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: base_url + "/ParishManagerfinal/setup/createDb",
                                    data: {format: 'JSON'},
                                    cache: false,
                                    success: function (result) {
                                        var response = JSON.parse(result);
                            ...
                            function dropDbConfirm() {
                                var dbname = document.getElementById('db_name').value;
                                swal({
                                    title: "Drop Database !",
                                    text: "Are you sure you want to drop database <strong>" + dbname + "</strong> ?",
                                    type: "warning",
                                    html: true,
                                    showCancelButton: true,
                                    closeOnConfirm: false,
                            ...
                            async function dropDb(dbname) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: base_url + "/ParishManagerfinal/setup/dropDb",
                                    data: {format: 'JSON', db: dbname},
                                    cache: false,
                                    success: function (result) {
                                        var response = JSON.parse(result);
                                        if (response.status == true) {
